...
## 发版兼容检查清单（2026-08-14 固化，修改前必查）

> 原则：修改时前置考虑发版会出现的问题，不允许「改完才发现生产跑不起来」。对照本清单逐项检查并登记风险与处置。

### ① 后端接口变更
- [ ] 新增接口：是否向后兼容（旧前端不调用 → 无影响）？是否需加入 `SecurityConfig` 白名单（公开接口）？
- [ ] 修改既有接口：是否破坏旧前端契约（字段名/响应结构/鉴权）？旧版本小程序还在线上，不能因新后端返回变化而报错
- [ ] 前后端同批次发布：前端调用新增接口，后端必须先上线（或同批），否则 404

### ② 数据库迁移
- [ ] 迁移 SQL **必须幂等**：可重复执行无副作用（`CREATE TABLE IF NOT EXISTS` / `INSERT IGNORE` / 条件 `DELETE`）
- [ ] **生产无表风险**：若代码查询的表在生产不存在（如 `actor_card_background`），必须提供**生产预置迁移**（建表 + seed），并把迁移写成「表存在才操作」的条件执行，任何环境安全
- [ ] 删除/清理数据：先确认无外键/无字段引用（如背景表行被卡以 URL 引用而非 id → 删行安全；删字段要查消费方）
- [ ] 本地执行用 `DbMigrationRunner apply-local`（127.0.0.1:3309）；`apply` 连远端（101.43.57.62:3306），生产迁移须用户下令后执行
- [ ] 后端对「表/字段缺失」做防御性容错（如 `BadSqlGrammarException` → 返回空/降级，不 500）

### ③ 前端产物 / 路由
- [ ] 新增路由（`pages.json`）：与旧产物自洽（小程序整包发布，新包含新页面，旧包完整不受影响）
- [ ] 组件/页面变更：包内自洽即可，删除旧组件不影响线上旧版本
- [ ] 发布产物用 `build:mp-weixin:release`（00-211），不含本地地址；`dist/dev` 仅本地联调
- [ ] 产物一致性：构建前退出微信开发者工具（00-216 结构性拦截已内置 sync 脚本）

### ④ 数据依赖
- [ ] 新功能依赖的数据（已发布卡/背景库/素材）在生产是否存在？缺失时前端空态/占位是否兜底（不报错）？

### ⑤ 回滚
- [ ] 迁移可逆（新增可删、清理可恢复备份）；接口新增可下线；前端整包可回退到上一版本
