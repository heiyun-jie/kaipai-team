...
## 14. Git 分支：默认全程留在 V3.0 开发

- 当前开发分支为 `V3.0`（2026-08-09 用户裁决）。默认所有改动直接在 `V3.0` 上开发与提交，禁止自作主张 `checkout` 到其它分支或新建特性分支。
- 该约束同时适用于本仓库与所有子仓库（`kaipaile-server` / `kaipai-frontend` / `kaipai-admin`）。
- 只有用户明确要求时才允许新建或切换分支；新建分支前必须先确认用户意图，不得按通用 git 习惯默认开分支。
- 若发现已在非 `V3.0` 分支，应先向用户说明并确认处理方式（合回 / 保留 / 删除），不得继续在该分支堆积改动。
- 历史约定曾指向 `main`，已于 `00-210` 轮次改为 `V3.0`；再遇到写着 `main` 的旧文档，以本条为准。
