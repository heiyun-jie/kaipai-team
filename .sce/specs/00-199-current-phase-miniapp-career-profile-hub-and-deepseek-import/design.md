...
| 路由 | 新职责 |
|---|---|
| `pages/mine/index` | 职业资料夹：账号头部、三个资产入口、我的作品集、联系申请、设置 |
...
共享暖米白页面背景 #F5F3EE（可渐变至 #F1EDE6）
  浅暖纸张 cell-group #FBFAF6
    从复制内容智能填写       标准单元格入口

  浅暖纸张 cell-group / 核心资料
    头像                     当前头像 + 进入素材库选择
    公开名称                 当前值 / 占位
    性别                     当前值 / 待完善
    年龄、身高               紧凑行级编辑
...
返回按钮必须复用 `KpFloatingBackButton` 的微信胶囊对齐、尺寸和命中区，删除页面私有的单字符 `‹`。导航栏固定使用共享 `surface`，不增加 Hero、独立营销渐变或第二个大标题。“从复制内容智能填写”保持工具入口层级，不使用营销大卡。首屏不得重新放入照片、视频、作品、PDF、完成度或提升建议。
...
| Token | 值 |
|---|---|
| 页面底色 | 共享 `bg #F5F3EE`；页面根可使用与“我的”页同款、以 `#F1EDE6` 为终点的极浅渐变 |
| 导航 / 分组背景 | 共享 `surface #FBFAF6` |
| 辅助背景 | 共享 `surface2 #EEEBE3` |
| 主文字 | 共享 `ink #1A1816` |
| 强调 / 选中操作 | 共享 `primary #8C6F4F` |
| 固定底部主操作 | 共享 `dark action #1D1814`，使用白色文字 |
| 分割线 / 边框 | 复用现有共享 `divider / border` Token，不新增页面私有冷灰色值 |
| 页面分组 | 无圆角、无阴影、无渐变；根背景的同款极浅渐变除外 |
| 按钮 | `6-8px` 圆角 |
| 底部选择层 | 顶部 `12-16px` 圆角、轻微上投影 |
| 布局 | 分组间距约 `12px`，行高 `48-56px`，左右边距约 `16px` |
| 字级 | 页面标题 `16-17px`，行标题 `15-16px`，说明 `12-13px` |

表单区不使用大号衬线标题；除页面根背景可复用“我的”页同款极浅渐变外，导航、分组和操作区不使用渐变、装饰性阴影或卡片堆叠。
...
#### `pkg-profile` 暖米白紧凑操作页视觉合同

四个 `pkg-profile` 页面共享档案页与“我的”页同源的暖米白操作壳层，页面之间不得继续保留冷灰、黑橙或衬线标题等历史主题差异：

| 元素 | 固定合同 |
|---|---|
| 导航 | `KpFloatingBackButton` + 固定 `surface #FBFAF6` 导航栏 + 居中短标题；不新增 Hero，不使用黑色头部 |
| 页面底色 | 共享 `bg #F5F3EE`；页面根可使用与“我的”页同款、以 `#F1EDE6` 为终点的极浅渐变 |
| 内容组织 | `surface #FBFAF6` 无圆角分组，分组之间留约 `12px`；辅助层使用 `surface2 #EEEBE3`，列表行与表单行复用共享 `divider / border` Token |
| 文字 | 主文字使用 `ink #1A1816`；页面标题约 `16-17px`、行标题 `15-16px`、说明 `12-13px`；不使用 display serif 或大号营销标题 |
| 操作 | 强调 / 选中态使用 `primary #8C6F4F`；固定底部主操作使用 `dark action #1D1814` 和白色文字，按钮保持 `6-8px` 圆角；次级操作使用 `surface / surface2` 与共享边框 |
| 禁止项 | 页面私有冷灰、黑橙主题、主体阴影、卡片堆叠、黑色导航和大号营销 Hero；除根背景同款极浅渐变外，不扩散渐变 |

视觉层只替换页面壳层和样式：识别复核、作品筛选与代表作、素材关系、上传、处理状态、错误提示、返回确认和固定底部操作的业务合同保持不变。
...
我的资料
  ...
  素材库    照片 / 视频 / 当前 PDF
常用功能
  我的作品集  -> /pkg-card/portfolio/index
  联系申请  待处理 badge
  设置
```
