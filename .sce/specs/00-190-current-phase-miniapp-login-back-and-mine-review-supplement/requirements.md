...
**描述**：基于 `00-189` E2E 结果，补充记录 `pages/mine/index` 的复核范围，明确个人中心已查看；同时确保未登录用户点击底部“我的”Tab 时可先停留在个人中心，并看到页面主要内容。

**验收标准**：

- WHEN 未登录用户点击底部“我的”Tab THEN 小程序应停留在 `pages/mine/index`，不得在 `onShow` 阶段直接跳转 `pages/login/index`。
- WHEN 未登录用户进入 `pages/mine/index` THEN 页面展示“未登录用户”和“登录后查看账号数据”游客态卡片。
- WHEN 未登录用户进入 `pages/mine/index` THEN 页面仍展示“我的数据”、快捷入口和设置项，不得只显示登录卡片和空白内容。
- WHEN 未登录用户点击编辑、创建分享、我的二维码、作品集、联系申请、收藏、通知、偏好设置等账号功能 THEN 再跳转登录页。
...
- WHEN `pages/mine/index` 游客态重新只显示登录卡片、不显示“我的数据 / 快捷入口 / 设置项” THEN 脚本失败。
...
- 本轮只做登录页局部 UI 和文档补充，不改登录 API、session 恢复、首页游客态或个人中心业务逻辑。
