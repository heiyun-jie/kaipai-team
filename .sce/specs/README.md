...
| 00-200 | current-phase-deepseek-profile-import-prompt-template-governance | 当前阶段 DeepSeek 资料识别 Prompt 模板治理：在 00-199 已有模型配置与提取/apply 主链上，新增 full_profile / works_only 场景草稿、固定样例试运行、不可变发布版本、历史恢复、独立权限审计和调用谱系；Phase A 后端持久化、Prompt Policy / Renderer、fail-closed Resolver、严格 DTO / list-detail 合同、固定 reasonCode、草稿生命周期、固定样例试运行、原子发布、不可变绑定审计、历史恢复与必写全局日志已完成，正在进入管理 API、严格权限与 V002 TDD | requirements.md, design.md, tasks.md |
