...
- [x] T56 已将本地裸 Java 微信联调收口到 `.sce/tools/start-kaipai-local-backend.ps1 -Restart`：启动前加载并校验 gitignored 微信 secret，以进程级临时环境变量安全注入，固定本地 MySQL / Redis 与 `NACOS_ENABLED=false` 边界，并以 `GET /api/doc.html=200` 作为有界基础健康门禁；**Validates: Requirements R7.5, R7.6, R7.6.1, R16, R16.1**
- [x] T57 收口后端运行时诊断的环境变量双层脱敏、部分证据留存、Docker duration 正规化、正则 grep 与 logging config 摘要，并补 Python 回归测试；**Validates: Requirements R14.6, R16.2, R16.2.1, R16.2.2, R16.2.3, R16.2.4**
