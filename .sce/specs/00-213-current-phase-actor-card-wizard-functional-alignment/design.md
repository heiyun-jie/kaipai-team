...
按 D5 逐项处置。A1（同步开关）已在 T3 实现、A2（作品落库）已在 T1 实现，本任务处置其余 7 项。

**实现 / 修正行为**

- **A3 剧照管理面板**：根因不是「面板缺失」而是 `addStill` 在满 3 张时 `count` 算成 `0`，`uni.chooseImage` 静默不弹窗 —— 原「管理剧照」按钮点了必然无反应。加 `remain <= 0` 前置判断并提示「请先删除」。同时删掉与之重复的「管理剧照」入口：行内的添加 / 删除控件本就在同一张卡上，两套入口做同一件事。顺带清掉 `已选N/3` 那个三元的 `false` 分支 —— 外层 `v-if="work.selected"` 已保证条件为真，该分支是死代码。张数上限提为 `MAX_STILLS` 常量，与后端 `MAX_STILLS_PER_WORK` 及 DTO `@Size` 对齐。
- **A4 展示顺序**：`order` 此前只写不读，用户调完顺序保存、返回本页看到的仍是默认排列 —— 表现为「排序没生效」。补 `applySavedOrder`：按存档 `order` 重排，存档中缺失的 key 追加尾部（后续新增模块不会因未知 key 丢项），数量不符则整体不动。

**移除 UI（能力登记为已知缺口）**

- **A5 拖拽排序**：「调整顺序」链接与「长按照片可调整展示顺序」文案均无实现支撑，一并移除。文案改为陈述现有行为「照片按添加顺序展示，第 1 张为封面」—— 序号角标本就是展示顺序，这句话是真的。
- **A6 素材库入口**：后端 `getActorAssets` 可用，但前端没有素材选择器组件，入口只能弹「即将开放」。移除入口，保留「从手机上传」。
- **A7 附件文件选择**：移除后步骤 6 出现结构问题 —— 页面只剩「跳过」和「下一步」两个按钮，而无附件时两者行为完全相同，是同一个动作的两个按钮。故整页重构：无附件时只留一个「下一步」，正文改为明示「附件简历暂未开放，该步骤为可选项，可直接继续」。已选附件卡片保留（含删除），因为历史草稿可能已有 `attachmentUrl`，删掉这段会连带丢掉历史数据的删除能力。

**推翻设计判断：A8 保存草稿**

§4.4 原判「零请求」为缺陷、须「补真实请求」。实施时核查后认为该判断有误，未按原文实施：

- 走到 `generate` 页意味着步骤 1/2/3/7 都已各自 `saveStep` 落库（`saveStep` 直接 `flushSave`，非 debounce）；
- 卡的 `status` 自 `createDraft` 起即为 `draft`（`ActorCardDraftServiceImpl:41`），无需额外置位；
- `runGenerate` 成功时已把 `generatedPreviewUrl` 写回 `actor_card`（`ActorCardGenerateService:93-96`）；
- 本页不采集任何用户输入，`scheduleSave` 全仓库无调用点，不存在未 flush 的 debounce 状态。

即此刻没有任何未落库的数据，补请求只会把已有值原样写回一遍。真正的缺陷在文案：「保存草稿」+ `icon: 'success'` 暗示此刻才发生保存。改为按钮「稍后再发布」、提示「草稿已在名片夹」，陈述既有事实。已核 `card-list` 的「草稿」Tab 确实按 `status='draft'` 拉取，草稿可被找回。

**随生成引擎登记（A9）**

`style` 落库但后端零消费、`backgroundImageUrl` 后端零读取点。UI 控件本身响应正常（能选中、能存值），缺的是下游消费方，属 `runGenerate` TODO 占位的同一个根因。与 C2 一并留待生成引擎实现，本轮不动代码。
