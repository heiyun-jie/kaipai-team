...
check(
  ...
  !/微信登录|微信一键登录|微信授权/.test(loginSource + runtimeSource),
  'Use self-owned copy such as 手机号快捷登录 instead of 微信登录 wording.',
);

check(
  'login page does not expose quick phone authorization entry',
  !/getPhoneNumber|手机号快捷登录|phone-quick/.test(loginSource),
  'The review baseline removes the quick phone authorization entry and keeps SMS verification as the visible login path.',
);
...
for (const distRoot of ['kaipai-frontend/dist/build/mp-weixin', 'kaipai-frontend/dist/dev/mp-weixin']) {
  ...
  if (!exists(loginWxml) || !exists(homeJs)) {
