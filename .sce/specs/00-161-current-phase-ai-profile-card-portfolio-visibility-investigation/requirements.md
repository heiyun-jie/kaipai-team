...
## Addendum: Mini Program Actor Detail Agent Contract

The AI output must become a visual asset for a complete mini-program actor detail page, not a model-rendered text poster.
...
2. The prompt agent must receive the selected style plus a fixed layout contract for the visual asset.
3. The prompt agent must use the user's profile information as visual guidance, but it must not ask the model to render final Chinese text, numbers, phone numbers, QR codes, buttons, or UI labels.
4. The final public experience must be rendered by mini-program native components so text remains accurate, copyable where allowed, clickable, and dynamically reusable.
5. The mini-program detail page must place real profile data into fixed modules:
...
7. The image stored in `actor_ai_profile_card_task.generated_image_url` and surfaced by portfolio/detail/share APIs is the AI visual asset/cover image for the native mini-program page.
...
9. A single static long-image renderer is allowed only as a future explicit `single-long-image` mode; it is not the default mode for this mini-program page.
