-- Platform admin and backend architecture DDL baseline v1
-- Notes:
...
CREATE TABLE `admin_role` (
  ...
  `status` TINYINT NOT NULL DEFAULT 1 COMMENT '1 active, 2 inactive',
  `remark` VARCHAR(255) DEFAULT NULL COMMENT 'remark',
  `menu_permissions_json` JSON DEFAULT NULL COMMENT 'menu permission codes',
  `page_permissions_json` JSON DEFAULT NULL COMMENT 'page permission codes',
  `action_permissions_json` JSON DEFAULT NULL COMMENT 'action permission codes',
  `version` INT NOT NULL DEFAULT 0,
  `deleted` TINYINT NOT NULL DEFAULT 0,
  `rid` VARCHAR(64) DEFAULT NULL,
  `create_user_id` BIGINT DEFAULT NULL,
...
CREATE TABLE `template_publish_log` (
  ...
  `template_id` BIGINT NOT NULL COMMENT 'template id',
  `target_type` VARCHAR(32) NOT NULL DEFAULT 'template' COMMENT 'template, theme_token, share_artifact',
  `target_code` VARCHAR(64) DEFAULT NULL COMMENT 'template code, theme code or artifact type',
  `publish_version` VARCHAR(32) NOT NULL COMMENT 'publish version',
  `draft_version` VARCHAR(32) DEFAULT NULL COMMENT 'draft version when publishing',
  `source_version` VARCHAR(32) DEFAULT NULL COMMENT 'source version before rollback',
  `target_version` VARCHAR(32) DEFAULT NULL COMMENT 'target version after rollback',
  ...
  `diff_summary_json` JSON DEFAULT NULL COMMENT 'diff summary json',
  ...
  KEY `idx_template_publish_log_target_type_published_at` (`target_type`, `published_at`),
  KEY `idx_template_publish_log_action_type` (`action_type`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='content publish logs';
