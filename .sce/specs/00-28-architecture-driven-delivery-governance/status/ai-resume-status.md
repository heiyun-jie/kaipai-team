...
- 后台已可对失败样本执行“人工复核 / 建议重试 / 关闭归档”三类治理动作，并记录处理状态、备注、处理人和处理时间
...
- 已新增的后台治理能力：`overview -> histories -> detail -> failures(filter) -> sensitive-hits(filter) -> review / suggest-retry / close -> handling timeline -> governance audit(filter)` 可回看 quota 消耗、最近历史、patch / snapshot 详情、失败样本、敏感命中、治理动作，并做最小人工处置
...
| 后台治理入口可操作 | 部分满足 | 后台已新增最小 AI 治理页，可查看概览、历史、详情、失败样本和敏感命中，并支持人工复核 / 建议重试 / 关闭归档；但仍缺更细治理动作与真环境验证 |
...
- 失败样本当前已支持筛选与备注时间线，并已补“关闭归档”状态；但仍未形成完整的分级处置状态流转
...
- 追加备注：`kaipaile-server` 已补失败样本查询条件、备注时间线与 `close` 处置状态；`kaipai-admin/src/views/system/AiResumeGovernanceView.vue` 已支持按条件筛选失败样本 / 敏感命中、执行关闭归档，并直接回看每次人工处置记录
