...
- 回填日期：`2026-04-02`
- 当前判定：`局部完成`
- 一句话结论：小程序邀请页、登录页邀请码承接、演员端 `/invite/*` 查询接口，以及服务端注册写入 `invitedByUserId / referral_record` 的最小闭环已继续收口，`/level/info` 与登录态的有效邀请数也开始回到 `referral_record` 单一事实源；`kaipai-admin` 的记录页 / 风险页 / 规则页 / 资格页与菜单路由权限也已接上，但真实环境联调、二维码真实生成和资格流转闭环仍未验证。
