...
3. 先以 `python .sce/runbooks/backend-admin-release/scripts/read-local-wechat-config-inputs.py --label <label>` 固定“本地是否具备合法输入”，再以 `python .sce/runbooks/backend-admin-release/scripts/read-backend-wechat-config-precheck.py --label <label>` 固定远端门禁结论；只有在拿到合法 secret 输入后，才继续按 `00-29` 补齐 compose 与 Nacos 的微信配置来源，随后补微信小程序 `appid / secret / getUnlimited` 与真实扫码落地证据，避免继续把“链接二维码可用”误判成“小程序码闭环完成”
