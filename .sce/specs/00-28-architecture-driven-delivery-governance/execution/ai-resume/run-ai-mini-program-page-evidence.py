...
def load_source_summary(sample_dir: Path) -> dict:
    ...
    if not actor_user_id:
        raise RuntimeError(f"source sample missing actorUserId: {sample_dir}")
    return {
...
def write_summary(sample_root: Path, source_summary: dict, manifest: dict) -> None:
    lines = [
        f"# AI Resume Mini Program Page Evidence {sample_root.name}",
        "",
        ...
        f"- History ID: `{source_summary.get('historyId')}`",
        f"- Failure ID: `{source_summary.get('failureId')}`",
        "",
        "## Captures",
        "",
    ]
...
def main() -> int:
    ...
    label = sys.argv[2].strip() if len(sys.argv) > 2 and sys.argv[2].strip() else "ai-mini-program-page-evidence"
    ...
    if process.returncode != 0:
        raise RuntimeError(
            "ai mini program capture failed: "
