...
const WINDOW_CAPTURE_SCRIPT = path.join(__dirname, 'capture-wechatdevtools-window.ps1')
...
async function switchRole(baseUrl, token, role) {
  const switched = await requestJson(`${baseUrl}/user/role`, {
    method: 'PUT',
    headers: {
  ...
  return {
    token,
    user: normalizeUserInfo(switched),
  }
}
...
async function ensureRoleSession(miniProgram, baseUrl, token, role) {
  const session = await switchRole(baseUrl, token, role)
  await withTimeout(`injectSession-role-${role}`, injectSession(miniProgram, session))
  await sleep(1000)
  const currentPage = await withTimeout(`currentPage-role-${role}`, miniProgram.currentPage())
  return {
    session,
    currentPage,
  }
...
async function capturePage(miniProgram, item, screenshotDir, captureDir) {
  appendProgress(captureDir, 'target-start', { name: item.name, role: item.role, path: item.path })
  ...
  appendProgress(captureDir, 'target-page-data-ok', {
    name: item.name,
    pageDataPath,
    pageDataKeys: Object.keys(pageData).slice(0, 12),
  })
...
async function captureTargetWithFreshConnection({
  ...
  try {
    const beforeCapturePage = await withTimeout(`beforeCapture-currentPage-${target.name}`, miniProgram.currentPage())
    appendProgress(captureDir, 'target-connection-page', {
      name: target.name,
      path: beforeCapturePage.path,
      query: beforeCapturePage.query,
    })

    const roleSessionState = await ensureRoleSession(miniProgram, baseUrl, token, target.role === 'actor' ? 1 : 2)
    appendProgress(captureDir, 'target-role-session-ok', {
      name: target.name,
      role: target.role,
      path: roleSessionState.currentPage.path,
      query: roleSessionState.currentPage.query,
    })
...
function buildTargets(roleId, applyId) {
  return [
    {
      role: 'crew',
      name: 'crew-home-projects',
      fileName: 'crew-home-projects.png',
      path: '/pages/home/index',
      waitForMs: 5000,
    },
    {
      role: 'crew',
      name: 'crew-apply-manage',
      fileName: 'crew-apply-manage.png',
      path: `/pages/apply-manage/index?roleId=${roleId}`,
      waitForMs: 5000,
    },
    {
      role: 'actor',
      name: 'actor-home-archive',
      fileName: 'actor-home-archive.png',
      path: '/pages/home/index',
      waitForMs: 5000,
    },
    {
      role: 'actor',
      name: 'actor-role-detail',
      fileName: 'actor-role-detail.png',
      path: `/pages/role-detail/index?id=${roleId}`,
      waitForMs: 5000,
    },
    {
      role: 'actor',
      name: 'actor-apply-confirm',
      fileName: 'actor-apply-confirm.png',
      path: `/pages/apply-confirm/index?roleId=${roleId}`,
      waitForMs: 5000,
    },
    {
      role: 'actor',
      name: 'actor-my-applies',
      fileName: 'actor-my-applies.png',
      path: '/pages/my-applies/index',
      waitForMs: 5000,
    },
    {
      role: 'actor',
      name: 'actor-apply-detail',
      fileName: 'actor-apply-detail.png',
      path: `/pages/apply-detail/index?id=${applyId}`,
      waitForMs: 5000,
    },
  ]
}
...
async function main() {
  ...
  const roleId = String(process.argv[7] || '')
  const applyId = String(process.argv[8] || '')
  const sourceSampleId = String(process.argv[9] || '')
  const captureManifestName = process.argv[10] || DEFAULT_CAPTURE_MANIFEST_NAME

  if (!roleId || !applyId || !actorUserId) {
    throw new Error('actorUserId, roleId, and applyId are required')
  }
  ...
  let beforeLoginPage = null
  let actorSessionPage = null
  let crewSessionPage = null
  let initialRole = null
  let initialSession = null

  const bootstrapMiniProgram = await connectMiniProgram(wsEndpoint, captureDir, 'bootstrap')
  try {
    beforeLoginPage = await withTimeout('beforeLogin-currentPage', bootstrapMiniProgram.currentPage())
    appendProgress(captureDir, 'before-login-page', { path: beforeLoginPage.path, query: beforeLoginPage.query })

    initialSession = await createSession(baseUrl, phone)
    initialRole = initialSession.user.role
    appendProgress(captureDir, 'session-created', { userRole: initialSession.user.role, userId: initialSession.user.id })

    await withTimeout('injectSession-initial', injectSession(bootstrapMiniProgram, initialSession))
    appendProgress(captureDir, 'session-injected', { role: initialSession.user.role })

    const actorSessionState = await ensureRoleSession(bootstrapMiniProgram, baseUrl, initialSession.token, 1)
    actorSessionPage = actorSessionState.currentPage
    appendProgress(captureDir, 'role-session-ok', { role: 'actor', path: actorSessionState.currentPage.path, query: actorSessionState.currentPage.query })

    const crewSessionState = await ensureRoleSession(bootstrapMiniProgram, baseUrl, initialSession.token, 2)
    crewSessionPage = crewSessionState.currentPage
    appendProgress(captureDir, 'role-session-ok', { role: 'crew', path: crewSessionState.currentPage.path, query: crewSessionState.currentPage.query })
  } finally {
    disconnectMiniProgram(bootstrapMiniProgram, captureDir, 'bootstrap')
  }

  const targets = buildTargets(roleId, applyId)
  const captures = []
  ...
  const result = {
    ...
    actorSessionPage: {
      path: actorSessionPage.path,
      query: actorSessionPage.query,
    },
    crewSessionPage: {
      path: crewSessionPage.path,
      query: crewSessionPage.query,
    },
    visualReview: {
