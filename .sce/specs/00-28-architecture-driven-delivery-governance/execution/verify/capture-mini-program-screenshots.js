...
const WINDOW_CAPTURE_SCRIPT = path.join(__dirname, '..', 'recruit', 'capture-wechatdevtools-window.ps1')
...
async function ensureActorSession(miniProgram, session) {
  await withTimeout('injectSession-actor', injectSession(miniProgram, session))
  await sleep(1000)
  return await withTimeout('currentPage-actor', miniProgram.currentPage())
}
...
async function collectActorApis(baseUrl, token, expectedUserId) {
  const verifyStatus = await requestJson(`${baseUrl}/verify/status`, {
    method: 'GET',
    headers: {
      Authorization: `Bearer ${token}`,
    },
  })
  const levelInfo = await requestJson(`${baseUrl}/level/info`, {
    method: 'GET',
    headers: {
      Authorization: `Bearer ${token}`,
    },
  ...
  if (String(userMe.userId) !== String(expectedUserId)) {
    throw new Error(`session user mismatch: expected=${expectedUserId} actual=${userMe.userId}`)
  }
...
async function capturePage(miniProgram, item, screenshotDir, captureDir, apiSnapshot) {
  ...
  const pageDataPath = path.join(captureDir, `page-data-${item.name}.json`)
  writeJson(pageDataPath, {
...
function buildTargets(userId) {
  return [
    {
      name: 'verify-page',
      fileName: 'verify-page.png',
      path: '/pkg-card/verify/index',
      waitForMs: 6000,
...
async function main() {
  ...
  const phone = String(process.argv[5] || '')
  const expectedUserId = String(process.argv[6] || '')
  const sourceSampleId = String(process.argv[7] || '')
  const captureManifestName = process.argv[8] || DEFAULT_CAPTURE_MANIFEST_NAME

  if (!phone || !expectedUserId) {
    throw new Error('phone and expectedUserId are required')
  }
  ...
  let session = null
  const miniProgram = await connectMiniProgram(wsEndpoint, captureDir, 'bootstrap')
  try {
    beforeLoginPage = await withTimeout('beforeLogin-currentPage', miniProgram.currentPage())
    appendProgress(captureDir, 'before-login-page', { path: beforeLoginPage.path, query: beforeLoginPage.query })

    session = await createSession(baseUrl, phone)
    appendProgress(captureDir, 'session-created', { userRole: session.user.role, userId: session.user.id })

    actorSessionPage = await ensureActorSession(miniProgram, session)
    appendProgress(captureDir, 'actor-session-ok', { path: actorSessionPage.path, query: actorSessionPage.query })

    const targets = buildTargets(expectedUserId)
    const captures = []
    for (const target of targets) {
      const apiSnapshot = await collectActorApis(baseUrl, session.token, target.expectedUserId)
      captures.push(await capturePage(miniProgram, target, screenshotDir, captureDir, apiSnapshot))
    }
    ...
    const manifestPath = path.join(captureDir, captureManifestName)
    ...
    disconnectMiniProgram(miniProgram, captureDir, 'bootstrap')
  }
}
