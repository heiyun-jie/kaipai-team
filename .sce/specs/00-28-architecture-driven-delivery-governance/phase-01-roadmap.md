...
但从 `2026-04-04` 起，前端当前阶段产品主入口已由 `00-62 current-phase-minimal-share-card-mvp-alignment` 重新定义：

- 首页只承接“可分享卡片 / 风格模板”
- 底部 Tab 收口为 `可以分享的卡片 / 查看历史 / 个人中心`
- `用户档案` 与 `分享卡片` 显式拆分
- 新用户默认拥有 `普通` 卡，其他风格由后台可配邀请阈值解锁
- 联系方式必须走 `申请 -> 同意 -> 查看` 闭环
...
- 第一优先级：继续以 `00-62 current-phase-minimal-share-card-mvp-alignment` 为入口，把当前已落位的首页、历史、个人中心三 Tab、独立持卡实体、查看历史和联系方式治理继续收口到 `shareCardId-first` 主键，不再长期停留在 `actorId + sceneKey` 兼容态。
- 第二优先级：以 `00-62` + `00-52` 为组合入口，继续补齐默认普通卡后台初始化 / 补偿策略表达、风格模板可见但锁定、后台可配邀请阈值，以及公开卡详情和后台联系方式治理的同批次真实环境样本。
- 第三优先级：membership 继续以 `00-49 membership-preview-overlay-fact-source-boundary` 作为治理入口，但其后续代码推进要服从 `00-62` 的卡片列表和单卡编辑新结构；没有新证据前，不再新开 overlay 后端化实现项。
- 第四优先级：login-auth 当前阶段维持维护态；verify 仅在认证契约、审核页或 schema 变化时复用标准页面脚本复验；微信登录、官方 `wxacode` 与正式短信能力继续保留在 `00-48 / 00-51` 后续批次，不再占用当前 MVP 主排期。
- 第五优先级：AI 简历继续以 `00-60 current-phase-ai-governance-real-notification-foundation` 收口真实通知 vendor/bridge/LLM 缺口，但不再压过 `00-62` 当前产品主入口改造。
...
- auth runtime / explicit mock 补充说明：`00-58 current-phase-auth-runtime-boundary-alignment` 已删除前端 runtime capability 表，`00-61 current-phase-auth-explicit-mock-retirement` 又已删除 auth 显式 mock 主链；后续 auth 域只允许围绕真实 `/api/auth/*`、`/api/user/*` 与 `00-48 / 00-51` 的微信 / 正式短信门禁继续治理。
