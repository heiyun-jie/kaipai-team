...
| 邀请裂变与邀请资格闭环 | 局部完成 | 前台邀请页和后台治理已成形，但 `/api/invite/*` 与 `/referral` 契约断层未收口 |
| 会员能力与模板配置闭环 | 局部完成 | 后台治理较完整，但演员端 `membership / card` 接口为空，前台仍靠本地 resolver |
...
- 第三优先级：为会员 / 模板定义演员端统一输出模型，逐步替换前端本地 resolver
