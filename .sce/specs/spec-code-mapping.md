...
| | `.sce/specs/00-191-current-phase-miniapp-verify-status-500-fix/execution.md` | — | ✅ 已回填：历史发布证据、至少 3 次复发标记、当前 Mine 边界、本轮实现、最新验证矩阵与未执行远端边界 |
| | `.sce/specs/00-191-current-phase-miniapp-verify-status-500-fix/scripts/verify-miniapp-verify-status-500-fix.mjs` | — | ✅ 已更新：当前 `17/17` 覆盖关联码、单次读取、completion 隔离、bootstrap / 页面 generation、masked-only 与 stale/current 401 延迟交错 |
| | `kaipaile-server/src/main/java/com/kaipai/service/verify/impl/IdentityVerificationServiceImpl.java` | — | ✅ 已改造：`currentStatus()` 窄范围回落到 `user.realAuthStatus`；status / admin detail 只返回 canonical masked 值；submit 删除旧 provider 二次调用 |
...
| | `kaipaile-server/src/main/java/com/kaipai/model/verify/dto/IdentityVerificationDetailRespDTO.java` | — | ✅ 已改造：物理删除 `idCardNoCipher`，后台详情只保留验证后的 `idCardNoMasked` 与 canonical provider 字段 |
...
| | `kaipaile-server/src/test/java/com/kaipai/module/server/verify/service/impl/IdentityVerificationServiceImplTest.java` | — | ✅ 已扩展：10 项覆盖窄范围降级、用户查询异常不吞错、默认/记录态，以及 status / admin detail 对 cipher/hash/非法 masked 的零暴露 |
| | `kaipaile-server/src/main/java/com/kaipai/common/exception/GlobalExceptionHandler.java` | — | ✅ 已收口：未处理异常返回非敏感关联码，并以同码记录 method / URI / throwable |
| | `kaipaile-server/src/test/java/com/kaipai/common/exception/GlobalExceptionHandlerTest.java` | — | ✅ 已新增：全局异常关联码响应、唯一性、非泄露性和日志上下文回归用例 |
| | `kaipai-frontend/src/api/verify.ts` | — | ✅ 已扩展：实名 GET 与 submit 均支持页面级请求选项，允许关闭请求层 loading / 默认错误提示 |
| | `kaipai-frontend/src/stores/user.ts` | — | ✅ 已收口：`ActorSessionSnapshot(token + userId + revision)`、bootstrap Promise 所有权、Storage/Pinia 双重校验和 status / level / invite 旧响应拒写 |
| | `kaipai-frontend/src/utils/auth.ts` | — | ✅ 已收口：统一 `clearSession()`、Pinia memory clear handler 与请求级 `AuthSessionSnapshot(token + revision)` |
| | `kaipai-frontend/src/utils/request.ts` | — | ✅ 已收口：请求发出时捕获 auth snapshot，仅当前会话 401 可清 Storage / Pinia 并跳登录，旧账号 401 不影响新账号 |
| | `kaipai-frontend/src/pkg-card/verify/index.vue` | — | ✅ 已收口：单次状态读取、局部 loading/error/retry、completion 独立重试、提交响应直用，以及 hydration/completion/submit 三类 generation |
| | `kaipai-admin/src/types/verify.ts` | — | ✅ 已收口：删除 `VerifyDetail.idCardNoCipher` 客户端合同 |
...
| | `.sce/runbooks/backend-admin-release/scripts/kaipai-backend-release-helper.sh` | — | ✅ 已收口：容器 / compose 环境双层白名单脱敏、候选 compose 校验结果安全摘要和 Docker logging config 摘要 |
| | `.sce/runbooks/backend-admin-release/scripts/read-backend-runtime-logs.py` | — | ✅ 已收口：Docker duration 正规化、正则 grep、失败部分证据留存，以及容器 / compose 环境本地二次脱敏 |
| | `.sce/runbooks/backend-admin-release/scripts/run-backend-compose-env-sync.py` | — | ✅ 已收口：compose 同步记录使用同一白名单，并拒绝把完整候选 `docker compose config` 输出写入记录 |
| | `.sce/runbooks/backend-admin-release/scripts/tests/test_backend_runtime_diagnostics.py` | — | ✅ 已新增：运行态诊断与 compose 同步脱敏、时间窗、正则、部分证据和 logging 摘要回归测试 |
...
| | `.sce/tools/start-kaipai-local-backend.ps1` | — | ✅ 已实现：本地裸 Java 启动前校验 gitignored 微信配置，安全注入子进程并固定本地 MySQL / Redis / 8010 运行边界 |
...
| 00-199 miniapp-career-profile-hub-and-deepseek-import | `.sce/specs/00-199-current-phase-miniapp-career-profile-hub-and-deepseek-import/requirements.md` | — | 🟡 书面 Spec 已确认；Plan 1 / Plan 2 与 T4 / T5 已完成，T6 / T9 和整体门禁尚未核销 |
| | `.sce/specs/00-199-current-phase-miniapp-career-profile-hub-and-deepseek-import/design.md` | — | 🟡 已建立渐进式数据模型、提取 / apply、迁移与消费者切换设计；Prompt 版本治理由 00-200 独立承接 |
| | `.sce/specs/00-199-current-phase-miniapp-career-profile-hub-and-deepseek-import/tasks.md` | — | 🟡 Plan 1 / Plan 2、T4 / T5 已完成；Plan 3 / T6 有历史实现但缺全量门禁 |
| | `kaipai-frontend/src/pages/mine/index.vue` | — | 🟡 已进入职业资料夹主链；00-199 整体验收仍未完成 |
| | `kaipai-frontend/src/pages/actor-profile/edit.vue` | — | 🟡 已接入简化档案与智能导入主链；当前工作树仍有未提交收口修改 |
| | `kaipai-frontend/src/pkg-profile/**` | — | 🟡 已存在导入复核、作品库、作品编辑与素材库分包页面；T6 / T9 全量门禁尚未核销 |
| | `kaipai-admin/src/views/system/AiProfileImportConfigView.vue` | — | ✅ 已实现 DeepSeek 模型、密钥、测试、启停和审计配置；Prompt 版本面板待 00-200 实现 |
| | `kaipaile-server/src/main/java/com/kaipai/model/actor/**` | — | 🟡 档案 / 作品与素材关系已有分阶段实现；00-199 迁移总门禁未完成 |
| | `kaipaile-server/src/main/java/com/kaipai/service/ai/**` | — | 🟡 DeepSeek 提取、校验、调用审计与原子 apply 已实现；生产 Prompt 仍硬编码，待 00-200 治理 |
...
| | `.sce/specs/00-202-current-phase-production-database-to-local-restore/execution.md` | — | ✅ 已记录：实际哈希、结构 / 计数、脱敏锚点、API smoke 与已知兼容缺口 |
| | `.sce/specs/00-202-current-phase-production-database-to-local-restore/scripts/verify-production-restore-source.sql` | — | ✅ 已执行：生产来源与林夏账号 / 实名 / 分享资产只读门禁 |
| | `.sce/specs/00-202-current-phase-production-database-to-local-restore/scripts/verify-local-production-mirror.sql` | — | ✅ 已执行：暂存与目标库表、列、逐表计数、数据库对象和关键资产镜像门禁 |
| 00-203 local-backend-schema-compatibility-gate | `.sce/specs/00-203-current-phase-local-backend-schema-compatibility-gate/requirements.md` | — | ✅ 已完成：本地 schema 漂移故障、迁移、启动门禁与业务 smoke 验收合同 |
| | `.sce/specs/00-203-current-phase-local-backend-schema-compatibility-gate/design.md` | — | ✅ 已完成：仓库迁移历史 / SHA256 与关键表列双层门禁设计 |
| | `.sce/specs/00-203-current-phase-local-backend-schema-compatibility-gate/tasks.md` | — | ✅ 已核销：备份、迁移、门禁、重启与登录态回归任务 |
| | `.sce/specs/00-203-current-phase-local-backend-schema-compatibility-gate/execution.md` | — | ✅ 已记录：原始堆栈、备份哈希、8 个迁移、真实接口与日志证据 |
| | `.sce/tools/start-kaipai-local-backend.ps1` | — | ✅ 已深化：Java 启动前校验全部仓库迁移记录 / SHA256 和职业资料关键 schema |
| | `.sce/tools/tests/test_start_kaipai_local_backend_schema_gate.py` | — | ✅ 已新增：门禁顺序、迁移哈希、关键对象和 CLI 分隔符解析回归测试 |
| 00-204 miniapp-actor-profile-weight-visibility | `.sce/specs/00-204-current-phase-miniapp-actor-profile-weight-visibility/requirements.md` | — | ✅ 已完成：核心资料体重常驻展示、单一字段、数据库事实和接口合同 |
| | `.sce/specs/00-204-current-phase-miniapp-actor-profile-weight-visibility/design.md` | — | ✅ 已完成：复用 `career.weight`、schema 门禁和专项验证设计 |
| | `.sce/specs/00-204-current-phase-miniapp-actor-profile-weight-visibility/tasks.md` | — | ✅ 已核销：体重可见入口、数据库 / API 合同门禁与验证记录 |
| | `kaipai-frontend/src/pages/actor-profile/edit.vue` | — | ✅ 已改造：唯一体重输入移到核心资料年龄 / 身高行，继续绑定 `draft.career.weight`，其他内容不变 |
| | `.sce/tools/start-kaipai-local-backend.ps1` | — | ✅ 已改造：既有 `actor_profile.weight` 纳入本地 schema 关键列门禁 |
...
| 00-211 mp-package-audit-external-url-unblock | `kaipai-frontend/package.json` | — | ✅ 已新增 2 个脚本：`build:mp-weixin:release`（`set VITE_API_BASE_URL=https://api.kplyyk.com&&uni build -p mp-weixin`，`&&` 前不留空格避免值尾带空格）与 `postbuild:mp-weixin:release`（挂 `sync-mp-weixin.ps1 -SourceOnly`）。依据实测：npm post 钩子为**完整脚本名精确匹配**（探针 `a:b` 触发 `posta:b`、未触发 `posta`），故必须显式定义 release 侧钩子；进程内变量优先级高于全部 env 文件，一处注入即覆盖 4 个内联点 |
| | `kaipai-frontend/scripts/sync-mp-weixin.ps1` | 1-6, 97-100, 144-152 | ✅ 已改造：新增 `[switch] $SourceOnly`，三项归一化（`Remove-RetiredExternalAssets` / `Repair-GeneratedNumericTernaryLiterals` / `Normalize-GeneratedTextEncoding`）后 `exit 0`，跳过 `robocopy /MIR` 与 `Apply-LocalDevProjectConfig`，使发版构建不污染 `dist/dev`；`Remove-RetiredExternalAssets` 补 `github.com/dcloudio/uni-app/issues/[0-9]+` 剥离规则。**关键事实**：该脚本第 144-146 行在 robocopy **之前**对 `dist/build` 原地改写，是「DEV/PROD 分支」假象的真因（把 `` `https://vuejs.org/error-reference/#runtime-${type}` `` 换成 `"runtime-error-"+type`，致 `vendor.js` md5 在 `d285cd8`/`ce23755` 间确定性翻转）；也说明它承担 `00-207` 白屏合同，**任何绕过 postbuild 的发版路径都会绕过白屏修复** |
| | `kaipai-frontend/scripts/audit-mp-package.ps1` | 60-81, 146-160 | ✅ 已修两处结构缺陷：(1) URL 检查原用 `Write-Error` 逐条报错，在 `$ErrorActionPreference="Stop"`（第 6 行）下首条即成为终止错误，报告只出 1 条且 `exit 1` 都执行不到 → 改为收集后全量 `Write-Host`（相对路径 + 行号 + 命中内容）并置 `$externalUrlFailed` 标志；(2) URL 检查排在包体统计之前致 `2 MB` 门禁长期不可达 → 改为两项各自出报告、末尾 `if ($externalUrlFailed -or $hasError) { exit 1 }` 聚合判定。**包体统计首次成功打印**：主包 `421.09 KB`（`20.56%`）/ `pkg-actor-card` `81.28 KB` / `pkg-profile` `80.67 KB` / `pkg-tools` `39.21 KB` / `pkg-card` `31.64 KB`，总计 `653.89 KB`；`2 MB` 阈值与分包统计逻辑未改 |
| | `kaipai-frontend/src/utils/request.ts`, `src/utils/runtime.ts`, `src/api/actor-asset.ts` | 16 / 9 / 29,45 | 🔍 只读核实：4 个内联点全部读 `import.meta.env.VITE_API_BASE_URL`，`src` 内无任何硬编码地址（全量 grep `http://` 仅命中 6 个 SVG `xmlns`）；**未改动**。推翻前序「把 baseURL 收进环境配置」方向——它已经在环境配置里，真因是 `.env.local` 覆盖 `.env` |
| 00-212 miniapp-nav-title-and-back-button-unification | `kaipai-frontend/src/components/KpPageNav.vue` | 全文（新增） | ✅ 已新增：唯一顶部导航契约组件。内部调用 `getFloatingBackNavStyles()`（setup 期同步取值）完成占位 + 胶囊带绝对定位 + `right: 200rpx` 避让，并**自持 `position: relative`**——相对 `00-207` 的改进点：调用方 `__header` 不再需要记得声明定位上下文，漏写导致定位逃逸到页面根节点的失败模式被结构性消除。接口 `title` / `showBack` / `backText` / `back` 事件 / 默认插槽；只 `emit('back')` 不自行 `navigateBack`，保留调用方离开前确认拦截能力 |
| | `kaipai-frontend/src/pkg-actor-card/{create,step-visual,step-profile,step-works,step-photos,step-video,step-attachment,step-settings,generate}/index.vue` | 各页 header 段 + style 段 | ✅ 已改造 9 页（**缺陷本体**）：原 `__nav` 排在 `KpCapsuleSpacer` **之后走正常文档流**，致胶囊水平带整块空置、返回箭头与标题被下推到正文起点并与正文 `__h1` 相贴——用户截图的「位置错位」与「标题重复」是同一条成因。9 页统一换 `KpPageNav`，删除各页手写 `&__nav` / `&__back` / `&__title`，下内边距由 `__header` 的 `padding-bottom: 16rpx` 承担（`create` 由既有 `__progress-row` 承担）。`generate` 为唯一特例：返回文案 `‹ 修改` 经 `back-text="修改"` 承载，原 `text-align: center` 标题改为与其余 11 页一致的左对齐 |
| | `kaipai-frontend/src/pkg-actor-card/step-visual/index.vue` | header 段, style 段 | ✅ 已改造：`__prog-bar` 移出导航行、独立为 `__prog-row`（宽度由 `80rpx` 改为 `100%` 铺满内容宽），与同级 `create` 的 `__progress-row` 做法对齐；删除 `__title { flex: 1 }`；删除与导航标题完全重复的 `__h1` |
| | `kaipai-frontend/src/pages/{home,card-list,mine}/index.vue` | header 段, script 段, style 段 | ✅ 已归一：`00-207` 的手写绝对定位标题行（各页自抄 `top`/`height`/`right: 200rpx`）换用 `KpPageNav` + `:show-back="false"`。**标题字重保护**：三页标题为 `34rpx/700`、`36rpx/600`、`36rpx/700`，与组件默认 `32rpx/600` 不同，故经**默认插槽**承载原标题节点、保留各页原 class，实现「定位统一、视觉零变化」；各页删除 `getFloatingBackNavStyles` 导入与 `backButtonStyle` 局部变量 |
| | `kaipai-frontend/scripts/verify-miniapp-nav-title-unification.mjs` | 全文（新增） | ✅ 已通过 `69 / 69`，并经**反向注入验证**（临时插回 `step-visual` 的重复 `__h1` → 门禁转红 `68/1` 且 `exit 1`，还原后复绿），证明门禁非空转。断言含 12 页组件化、9 页无流式 `__nav`、5 页重复 `__h1` 消除、2 页改写 `__h1` 保留（防过度删除）、进度条移出导航、`200rpx` 魔数收敛、组件契约 7 项。按 `00-211` 教训**全量收集失败项后再非零退出**，不首错即停 |
| | `kaipai-frontend/package.json` | 8 | ✅ 已接入 `verify:nav-title`（按 `00-205` 教训：未接入 `package.json` 的脚本不计门禁） |
| | `.sce/specs/SHARED_CONVENTIONS.md` | 22, 85-105 | ✅ 已校正 2 处失效约定：第 22 行 `KpNavBar` 在 `00-209` 组件退场后已不存在 → 改为 `KpPageNav`；第 85 行「页面本地实现，不依赖共享导航组件」与本 Spec 冲突 → 拆为三段（标题型统一走 `KpPageNav` / 深色 Hero 页维持本地悬浮返回 / 普通顶部页现状未合并），并登记 `npm run verify:nav-title` |
| | `kaipai-frontend/src/components/KpCapsuleSpacer.vue` | — | 🔒 未改动：改造后仅 `pages/login/index.vue` 直接引用（该页无导航标题行，不在缺陷面内），其余 12 页改为经 `KpPageNav` 间接持有 |
| | `kaipai-frontend/src/components/KpFloatingBackButton.vue` | — | 🔒 未改动：模式 B（`actor-profile/edit` / `pkg-profile/import-review` / `pkg-profile/assets`）与模式 C（`pkg-card/verify` / `pkg-tools/webview` / `pkg-tools/video-player`）共 6 页在用；居中 + sticky 语义与 Hero 页策略均与 `KpPageNav` 不同构，本轮按设计边界不合并 |
| | `kaipai-frontend/vite.config.ts` | 9 | 🔍 只读核实：`minify: false` 为 `00-207` 合同（esbuild 产出 `?.5:` 非法语法致微信页面 JS 解析失败 / `navigateTo` timeout 白屏），**未改动**；本轮曾临时加诊断打点证明三脚本 `NODE_ENV` 全同，已清除（`KP_PROBE` 残留 `0`） |
| | `kaipai-frontend/.env.local` | — | 🔍 只读核实：`http://127.0.0.1:8010`，按非功能需求**不得修改**，本机联调不受影响；发版产物本机地址 `4 → 0` 而 `dist/dev` 仍为本机 `4` / 生产 `0`，`urlCheck=false` 仍生效 |
| | `kaipai-frontend/.env.example` | — | ⏸️ 本轮移出范围：被 `.gitignore:15` 的 `.env.*` 连带忽略、`git ls-files` 确认未入库，属独立缺陷，与发版阻塞无因果关系 |
| | `kaipai-frontend/src/pages/login/index.vue` | 16 | ⏸️ 本轮移出范围：`剧组版 · 分享平台` 为旧版定位残留（`git blame` = `74a60345` 黑云 `2026-05-23`，非合并带入），需用户给新文案 |
| 00-213 actor-card-wizard-functional-alignment | `kaipai-frontend/src/stores/actor-card-draft.ts` | 45-50, 60-69, 78-118, 92-95, 98-100 | 📋 待改造（**阻断 1 所在**）：`:92-95` 步骤 3 硬编码 `statusCode: 'empty'`，注释「由子页面动态更新」在 `computed` 架构下不成立（全仓 `stepStatuses` 4 处引用、零 setter），致 `:120` `requiredAllDone` 恒 `false`、`:124` `nextIncompleteStep` 恒返回 3；`:49` / `:66` 两处空 catch 使读取与保存失败均静默（保存失败后 resolve 而非 reject，各页无条件 `navigateTo`）；`:98-100` computed 内 `JSON.parse` 无保护会打挂整个 Hub 页 |
| | `kaipaile-server/src/main/java/com/kaipai/service/actor/ActorCardGenerateService.java` | 34, 88-90, 111-117, 130 | 📋 待改造（**阻断 2 所在**）：`:111-117` 要求 `actor_card_work` ≥ 1 行而全仓无写入路径，生成必然被拒；`:34` `STATUS_SUCCESS = "success"` 与前端 `'done'` 不一致；`:88-90` `// TODO: 接入真实长页渲染引擎`，`previewUrl` 直接取主视觉原图，资料/作品/照片/视频/附件/设置全不体现 |
| | `kaipaile-server/src/main/java/com/kaipai/service/actor/impl/ActorCardDraftServiceImpl.java` | 24-32, 35-49, 105-113 | 📋 待改造：`:24-32` `createDraft` 只设 userId/status/currentStep/publishedVersion，`profileSnapshotJson` 恒 null（`00-206 §3.4` 预填从未实现的后端侧成因）；`:38-43,47` 用 `hasText`、`:44-46` 用 `!= null`，同一方法两套口径致 7 字段无法清空；`:105-113` 完整度只累计 3 项却除以 7（满填 43%），注释 `settlingsJson` 拼写错误且所述「视为完成」未实现 |
| | `kaipaile-server/src/main/java/com/kaipai/service/actor/ActorCardExpandImageService.java` | 40, 85-97, 144 | 📋 待改造：`:40` `STATUS_SUCCESS = "success"`、`:144` 写入该值、`:92` `status()` 原样回传 `task.getStatus()` 无映射；前端只认 `'done'` 故轮询必然耗尽 |
| | `kaipai-frontend/src/pkg-actor-card/step-profile/index.vue` | 8, 28, 36, 55, 58-61, 64-68 | 📋 待改造（**本轮用户提问落点**）：`:8` 文案承诺「将自动填入」但 `:64-68` 只读草稿快照、从不拉演员资料；`:36`/`:55` 同步开关空转（`handleNext` 不读、未 import `updateMyActorProfile`、无落库字段故重进恒复位）；`:28` 联系方式无源无列无消费方 |
| | `kaipai-frontend/src/pkg-actor-card/step-works/index.vue` | 9, 89-93, 99-118, 135, 141-143 | 📋 待改造：`:89-93` 列表为 3 条硬编码示例（夏日未央/逆光而行/城市边缘）而 `/api/actor/works` 后端已存在、前端缺 `api/actor-work.ts`；`:135` 只提交 `currentStep`，作品/剧照/新增作品全量丢弃（注释自陈「通过 settingsJson 临时承接」而连该字段都没传）；`:116` 「管理剧照」实为追加、满 3 张时 `count:0` 点击无反应；`:141-143` 无任何回填 |
| | `kaipai-frontend/src/pkg-actor-card/step-visual/index.vue` | 9, 16, 26, 95, 103, 113-121, 125, 143, 162, 174-179 | 📋 待改造：`:143` 判 `'done'` 与后端 `success` 不一致；`:116` 临时路径落库并透传给需公网地址的 provider；`:162` `expandedImageUrl \|\| sourceImageUrl` 污染致步骤 1 恒 done、重进渲染两张相同图且扩图入口消失；`:9` 承诺「自动扩图」实需手点；`:103` 背景库失败降级空列表；`:95` 轮询定时器无清理；`:176-179` 背景回填依赖 URL 精确匹配且仅当前风格库、无兜底 |
| | `kaipai-frontend/src/pkg-actor-card/{step-photos,step-video,step-attachment,step-settings,generate,create}/index.vue` | 见 design.md §5 | 📋 待改造：`step-photos` 排序无实现 + 素材库占位 toast；`step-video` 存 `tempFilePath`；`step-attachment` 文件选择占位致该页所有控件不可达、步骤 6 恒「未添加」；`step-settings` `order` 单向写入（前端不回读、后端不解析）；`generate` 「保存草稿」零请求 + 三处早退致永久 loading + 轮询定时器不留句柄；`create:17` 文案承诺生成而三重阻断 |
| | `kaipai-frontend/src/api/actor-card.ts` | 88, 96 | 📋 待改造：状态类型声明为 `'pending'\|'running'\|'done'\|'failed'`，与后端实际返回 `success` 不符，且前端无归一化（grep `'success'` 于 api/utils/stores 命中 0） |
| | `kaipaile-server/src/main/java/com/kaipai/model/actor/card/dto/{ActorCardExpandImageRespDTO,ActorCardGenerateRespDTO,ActorCardStepSaveReqDTO,ActorCardRespDTO}.java` | 8 / 8 / 全文 / 32 | 📋 待改造：前两者注释写 `pending\|running\|done\|failed` 与实现 `success` 自相矛盾（缺陷长期未被发现的原因）；`ActorCardStepSaveReqDTO` 无 works / 同步开关字段；`ActorCardRespDTO.stepStatuses` 注释「Hub 页用」但 `toDto` 从未 set 恒 null，构成阻断 1 的根因 |
| | `kaipaile-server/src/main/java/com/kaipai/model/actor/card/entity/ActorCardWork.java`, `mapper/actor/ActorCardWorkMapper.java` | 全文 | 📋 待处置：表与 Mapper 齐备但**零写入路径**，全 server 仅 1 次 `selectCount` 读取，而该读取正是生成硬门禁 |
| | `kaipai-frontend/src/api/actor-work.ts` | — | 📋 待新建：`/api/actor/works` 与 `GET /api/actor/works/{id}/assets` 后端均已存在（`ActorWorkController:22`），前端缺该模块，是 `step-works` 只能用硬编码示例的直接原因 |
| | `kaipai-frontend/scripts/verify-actor-card-wizard-alignment.mjs` | — | 📋 待新建：7 组断言门禁，须收集全部失败项后再非零退出（`00-211` 教训）+ 接入 `package.json` 为 `verify:wizard-alignment`（`00-205` 教训）+ 反向注入证明非空转。**边界**：存在性 grep 对运行时行为结构性无效，端到端走通 / 枚举实际对齐 / provider 取图三项必须实测 |
| | `kaipai-frontend/src/api/actor.ts`, `src/api/actor-asset.ts`, `src/types/profile.ts` | 16,27,31 / 8,28 / 30-50 | 🔍 只读核实：预填与回写所需能力**全部已存在但向导零引用** —— `getOptionalMyActorProfile` / `getMyCareerProfile` / `updateMyActorProfile`（带 `expectedProfileVersion` 乐观锁）、`getActorAssets` / `uploadActorAsset`；`ActorProfileResp` 可供 `publicName`/`height`/`currentCity`/`schoolName`/`intro` 五字段，**无 phone/wechat/contact 任一字段**（「联系方式」无预填源已确认） |
| | `kaipai-frontend/src/pages.json` | — | 🔒 未改动：`pkg-actor-card` 9 页登记齐全，已核对无缺 |
| | `kaipai-frontend/src/components/KpPageNav.vue`, `src/pkg-actor-card/components/KpStepProgress.vue` | — | 🔒 未改动：属 `00-212` 导航壳层范围，本 Spec 只管功能本体（行为层），两者边界不重叠 |
| | `kaipaile-server/src/main/java/com/kaipai/service/actor/ActorCardPublishService.java` | 72-73, 118-127 | ⏸️ 部分移出范围：`:72-73` `materialCount(0)` / `viewCount(0)` 写死，影响面在个人中心页而非本向导 9 页，判定范围外仅登记；`:118-127` 完整度另一套口径（`:122` 作品项无条件 `done++`）与 Draft 侧矛盾，属 §3.9 范围内 |
|------|--------|------|------|
| 00-214 current-phase-actor-card-attachment-resume-end-to-end | `kaipaile-server/src/main/resources/db/migration/V20260810_001__actor_card_attachment_asset_binding.sql` | 全文（新增） | ✅ A1 DDL：`actor_card` 加 `attachment_asset_id BIGINT` + `idx_actor_card_attachment_asset` 索引；`actor_card` 已含 `attachment_url`（`@Deprecated` 停写只读）。本地 `DbMigrationRunner apply-local` exit 0，已按格式登记 `schema_release_history`（`release_id=...-local-schema-00-214`） |
| | `kaipaile-server/src/main/java/com/kaipai/model/actor/ActorCard.java` | — | ✅ A1：新增 `attachmentAssetId` 字段；`attachmentUrl` 标 `@Deprecated` 停写只读 |
| | `kaipaile-server/src/main/java/com/kaipai/service/actor/ActorMediaAssetOwnershipVerifier.java` | — | ✅ A2：新增 `requireOwnedReadyPdf` 抽象方法（刻意不给 default，逼每个实现显式表态） |
| | `kaipaile-server/src/main/java/com/kaipai/service/actor/impl/ActorMediaAssetServiceImpl.java` | — | ✅ A2：photo / pdf 校验收敛到私有 `requireOwnedReady(userId, assetId, expectedMediaType)`；错误码复用 `46012 PROFILE_ASSET_NOT_FOUND`（越权/不存在）/ `46013 PROFILE_ASSET_NOT_READY`（非 pdf / processing/failed）。单测 4 条全绿 |
| | `kaipaile-server/src/main/java/com/kaipai/integration/ActorMediaAssetOwnershipVerifierConfiguration.java` | — | ✅ A2：兜底 bean 两个方法都抛 `PROFILE_ASSET_NOT_FOUND`，改匿名类实现 |
| | `kaipaile-server/src/main/java/com/kaipai/service/actor/impl/ActorCardDraftServiceImpl.java` | 附件段 | ✅ A3：`saveStep` 三态语义 — 不传键 = 不变 / `{assetId:N}` = 调 `requireOwnedReadyPdf` 验归属后写 `attachmentAssetId` / `{assetId:null}` = 显式清空两字段（同时清 `attachmentUrl`）；步骤 6 完成度判据从 `hasText(attachmentUrl)` 切换到 `attachmentAssetId != null`；`getActorCardDraft` 响应填充 `attachmentAssetId` 派生字段 |
| | `kaipaile-server/src/main/java/com/kaipai/model/actor/card/dto/ActorCardStepSaveReqDTO.java` | — | ✅ A3：新增嵌套 `AttachmentBinding` DTO（`assetId` 可 null）；主 DTO 新增 `attachment` 字段 |
| | `kaipaile-server/src/main/java/com/kaipai/controller/api/actor/ActorMediaAssetController.java` | — | ✅ A4：新增 `GET /api/actor/assets/{id}/pages`，验归属、拒非 ready，逐页调 `getSignedAccessUrl` 签发 10 分钟预签名 URL，返回 `ActorAssetPageRespDTO` 列表 |
| | `kaipaile-server/src/main/java/com/kaipai/model/actor/asset/dto/ActorAssetPageRespDTO.java` | 全文（新增） | ✅ A4：页图 DTO（`pageIndex`, `accessUrl`, `width`, `height`） |
| | `kaipai-frontend/src/api/actor-asset.ts` | — | ✅ A6：新增 `listActorAssetPages(assetId, opts?)` → `GET /api/actor/assets/{id}/pages`；`api/actor-card.ts` 补 `AttachmentBinding` 类型与 saveStep 三态重载 |
| | `kaipai-frontend/src/pkg-actor-card/step-attachment/index.vue` | 全文（重写） | ✅ A7 + A8：完整三态 UI（C 上传区 / A 文件卡 / B 历史卡）。三态状态机：`assetId` + `assetStatus`（`'ready'\|'processing'\|'failed'\|null`）+ `legacyUrl`（向后兼容 `attachmentUrl`）。上传：`uni.chooseMessageFile({extension:['.pdf']})` 取消安全（`errMsg.includes('cancel')`）→ `uploadActorAsset` → `saveStep({attachment:{assetId}})` → 轮询（`POLL_INTERVAL=2500ms`，`POLL_MAX=24`，60 s 超时）。预览：每次重新调 `listActorAssetPages` 签发地址（不缓存）→ `uni.previewImage` 逐页浏览（A8）。替换 = 新选 + 上传，新 assetId 覆盖绑定。删除：`stopPoll()` + `saveStep({attachment:{assetId:null}})` + 清本地状态含 `legacyUrl`。`handleNext` / `handleSkip` 不传 `attachment` 键（no-touch 语义）。`onMounted` 按 `attachmentAssetId`→A / `attachmentUrl`→B / 空→C 三态恢复 |
| | `kaipai-frontend/src/pkg-actor-card/create/index.vue` | 70 | ✅ A9：步骤 6 提示文案 `'支持PDF、PPT、PPTX，最多1份'` → `'PDF 格式，单份，≤ 20 MB，≤ 20 页'`（与后端三重校验对齐，PPT/PPTX 从文案退场） |
| | `kaipai-frontend/scripts/verify-00214-actor-card-attachment.mjs` | 全文（新增） | ✅ A10：7 组共 17 项断言，全绿（17/0）：§5.1 无旧文案、§5.2 核心 API 函数引用、§5.3 全仓 saveStep 不含 `attachmentUrl` 赋值、§5.4 handleNext/handleSkip/navigate 不含 `attachment` 键、§5.5 create/index.vue 无 PPT/PPTX、§5.6 后端含 `requireOwnedReadyPdf` 与 `AttachmentBinding`、§5.7 三组反向注入自检（确认非空转）。收集全部失败项后非零退出（`00-211` 教训） |
| | `kaipai-frontend/package.json` | scripts | ✅ A10：接入 `"verify:actor-card-attachment": "node scripts/verify-00214-actor-card-attachment.mjs"`（`00-205` 教训） |
| | `kaipai-frontend/src/api/actor-card.ts`, `kaipai-frontend/src/stores/actor-card-draft.ts` | — | 🔒 未改动源文件结构：A6 为类型扩展与新增函数，现有函数签名未变；store `saveStep` 调用方只传新 `attachment` 字段，其余字段原样 |
